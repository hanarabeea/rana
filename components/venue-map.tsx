...
export default function VenueMap() {
  ...
  // Map URL for Jumeira Hall
  const googleMapsUrl = "https://maps.apple.com/place?address=Al%20Garby%20Street,%20Ras%20El%20Bar,%20Damietta,%20Egypt&coordinate=31.492711,31.825756&name=Jumeira%20Hall&place-id=I4B560E61A8D6456B&map=explore";
        ...
        <div className="relative group">
          ...
          <div className="mt-6 text-center px-4">
            <h3 className="text-xl sm:text-2xl font-serif font-medium text-foreground mb-2">
              {isRTL ? 'قاعة جميرا' : 'Jumeira Hall'}
            </h3>
            <div className="text-muted-foreground text-sm sm:text-base mb-3 space-y-1">
              {isRTL ? (
                <>
                  <div>رأس البر، دمياط</div>
                </>
              ) : (
                <>
                  <div>Ras El Bar, Damietta</div>
                </>
              )}
            </div>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <span>{isRTL ? 'دمياط، مصر' : 'Damietta, Egypt'}</span>
              </div>
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
